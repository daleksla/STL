...
namespace Salih::Algorithms::Sorts {
	...
	inline Salih::Structures::Contiguous::Array<T, SIZE+SIZE> merge(const Salih::Structures::Contiguous::Array<T, SIZE>& x, const Salih::Structures::Contiguous::Array<T, SIZE>& y, bool(*const order)(int, int) = ASCEND)
	...
	{	
		Salih::Structures::Contiguous::Array<T, SIZE+SIZE> out ;
	...
	template<typename T, std::size_t SIZE, std::size_t DIFF>
	inline Salih::Structures::Contiguous::Array<T, SIZE+DIFF> merge(const Salih::Structures::Contiguous::Array<T, SIZE>& x, const Salih::Structures::Contiguous::Array<T, DIFF>& y, bool(*const order)(int, int) = ASCEND)
	/** This is the merging subroutine - it works by taking two lists and adding the smallest / the largest value from each, such that the resulting list is a combined list in order
	 ...
	 * @return list (sorted version) **/	
	{	
		Salih::Structures::Contiguous::Array<T, SIZE+DIFF> out ;
		std::size_t i = 0, j = 0, k = 0 ;
	...
	template<typename T, std::size_t SIZE>
	Salih::Structures::Contiguous::Array<T, SIZE> mergeSort(const Salih::Structures::Contiguous::Array<T, SIZE>& data, bool(*const order)(int, int) = ASCEND)
	/** This is the merge sort algorithm. It works splitting a list into minimal sublists and combining two, in order, into a bigger list. It keeps merging the lists together, then the merged lists together, until a fully sorted list is formed
	 * @param const refernece to list (to be sorted)
	 * @param const function pointer, will facilitate ordering of list. Set as either ASCEND (1,...,n) or DESCEND (n,...,1)
	 * @return list (sorted version) **/	
	{			
		...
		Salih::Structures::Contiguous::Array<T,SIZE/2> a, b ;
