...
namespace salih::algorithms {
	...
	_GLIBCXX14_CONSTEXPR inline std::size_t getMiddle(const std::size_t size) ;
	...
	_GLIBCXX14_CONSTEXPR bool binarySearch(const T&, const U&) ;
	 ...
	 * @return boolean (indicates if supplied target was found or not) **/
	template<typename T, typename U>
	_GLIBCXX14_CONSTEXPR bool interpolationSearch(const T&, const U&) ;
