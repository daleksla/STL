...
namespace salih::algorithms {
	...
	_GLIBCXX20_CONSTEXPR inline std::size_t getMiddle(const std::size_t size) ;
	...
	_GLIBCXX20_CONSTEXPR bool binarySearch(const T&, const U&) ;
	 ...
	 * @return boolean (indicates if supplied target was found or not) **/
	template<typename T, typename U>
	_GLIBCXX20_CONSTEXPR bool interpolationSearch(const T&, const U&) ;
